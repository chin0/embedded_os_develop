...
dma_addr_t h5400_map_single(void *, size_t, int);
void h5400_unmap_single(dma_addr_t, size_t, int);
...
#define H5400_FAKE_PCIDEV ((struct pci_dev *) 0xffff5400)

static inline int dev_is_h5400(const struct pci_dev *dev)
{
	return (dev == H5400_FAKE_PCIDEV);
}

#else

static inline int dev_is_h5400(const struct pci_dev *dev) { return 0; }
...
pci_map_single(struct pci_dev *hwdev, void *ptr, size_t size, int direction)
{
	...
	if (dev_is_h5400(hwdev))
		return h5400_map_single(ptr, size, direction);
...
pci_unmap_single(struct pci_dev *hwdev, dma_addr_t dma_addr, size_t size, int direction)
{
	...
	if (dev_is_h5400(hwdev))
		return h5400_unmap_single(dma_addr, size, direction);
	/* nothing to do */
}
